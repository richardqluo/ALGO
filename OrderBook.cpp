...
struct Order{
    ...
    double price;
    Order(unsigned long long i, unsigned int tm, string sb, char tp, char sd, int sh, int p):id(i),time(tm),symbol(sb),type(tp),side(sd),shares(sh),price(p){}
};
